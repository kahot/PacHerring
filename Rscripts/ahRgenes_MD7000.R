...
win <- "100k"
step <- "10k"
freqs_win_p <- read.csv(paste0("Data/freqs/MD7000/PWS_shifts_100k_10k.txt"), row.names = 1)
freqs_win_t <- read.csv(paste0("Data/freqs/MD7000/TB_shifts_",win,"_",step,".txt"), row.names = 1)
freqs_win_s <- read.csv(paste0("Data/freqs/MD7000/SS_shifts_",win,"_",step,".txt"), row.names = 1)
...
geneset$chr<-paste0("chr",geneset$seqnames)

write.csv(geneset, "Output/ahR/ahRpathway_geneset.csv")


#Start with ahR 
i <- 2
#for (i in chrs){
freqs_win_p2<-freqs_win_p[freqs_win_p$chr_num%in% 2,]
...
ggplot(freqs_win_p2, aes(x = win_mid, y =zt01_mean,)) + 
    theme_minimal()+
    geom_line(aes(x = win_mid, y = zt01_mean, color="1991-1996"))+
    geom_line(aes(x = win_mid, y = zt12_mean, color="1996-2007"))+
    geom_line(aes(x = win_mid, y = zt23_mean, color="2007-2017"))+
    labs(x = "", y = expression(paste(Delta, "z")),color = "") +
...
ggsave("Output/ahR/MD7000/AHR1b_PWS_100k_AFshift.pdf", width=9.5, height =6)
...
ggsave("Output/ahR/MD7000/AHR1b_PWS_100k_AFshift_smooth.pdf", width=9.5, height =6)
...
ggplot(all_pops2m[all_pops2m$loc %in% c("p","t"), ], aes(x = win_mid, y = value, color=year, linetype=loc)) + 
    ...
    xlim(ahr1b$start-50000, ahr1b$start+50000)+
    ggtitle("AHR1b")
ggsave("Output/ahR/MD7000/AHR1b_PWS_TB_shifts.pdf", width = 9, height = 8)
...
ggplot(all_pops2m, aes(x = win_mid, y = value, color=year, linetype=loc)) + 
    ...
    xlim(ahr1b$start-50000, ahr1b$start+50000)+
    ggtitle("AHR1b")
ggsave("Output/ahR/MD7000/AHR1b_PWS_TB_SS_shifts.pdf", width = 9, height = 8)
...
ggplot(all_pops2m, aes(x = win_mid, y = value, color=year, linetype=loc)) + 
    ...
    xlim(ahr1b$start-50000, ahr1b$start+50000)+
    ggtitle("AHR1b")
ggsave("Output/ahR/MD7000/AHR1b_PWS_TB_SS_shifts_smooth.pdf", width = 9, height = 8)
