...
ch="chr16"
dir.create("Output/chr/chr16/")
vcf <- read.vcfR("Data/new_vcf/PH_MD7000_maf05_chr16.vcf.gz")
...
pdf(paste0("Output/chr/DP7000/",ch,"/",ch,"_SS-all.pdf"), width =8, height = 16 )
ggdraw()+
...
pdf(paste0("Output/chr/DP7000/",ch,"/",ch,"_CA.WA.BC.pdf"), width =8, height = 16 )
ggdraw()+
    draw_plot(combine_genotype_plot(chr_plot1),x=0,y=0.65,width=1,height=0.35)+
    draw_plot(chr_plot2$genotypes,0,0.35,1,0.3)+
    draw_plot(chr_plot3$genotypes,0,0,1,0.35)
...
ca_plot1 <- genotype_plot(vcf_object  =  vcf,   
                           popmap = ca,                           
                           snp_label_size = 1000000,                     
                           colour_scheme=c(yel,"red","blue"),
                           plot_allele_frequency=TRUE,                    
                           invariant_filter = F)   
pdf(paste0("Output/chr/DP7000/",ch,"/AF_,",ch,"_CA17.pdf"), width = 10, height = 5)
combine_genotype_plot(chr16_plot)
dev.off()


ca_plot <- genotype_plot(vcf_object  =  vcf,   
                            popmap = ca,                           
                            snp_label_size = 1000000,                     
                            colour_scheme=c(yel,"red","blue"),
...
gt<-ca_plot$genotypes[["data"]]
positions<-data.frame(getFIX(vcf))

gt$pos<-rep(as.integer(positions$POS), each=70)
gt$Sample<-ca$Sample

write.csv(gt, "Output/chr/DP7000/chr16/chr16_CA_genotype_info.csv")
...
gty2<-gty[gty$pos>start&gty$pos<end,]
ggplot(gty2, aes(x=pos, y=value, color=variable, fill=variable))+
    geom_bar(stat="identity", position="fill")+
    scale_x_continuous(breaks=seq(start,end, 100000))+
    theme(axis.text.x = element_text(angle=90))+
    theme(legend.title = element_blank())
ggsave(paste0("Output/chr/DP7000/",ch,"/Genotype_plot_nearInversion_",ch,".pdf"), width=7, height = 5)



####################
# Find individuals with inversions    
ca17<-read.csv("Output/chr/DP7000/chr16/ /CA17_gtinfo.csv", row.names = 1)
ca17<-gt
#select snps in inversion regions 
ca2<-ca17[ca17$snp<4000000,]
#number of snps 
...
#44 individuals
write.csv(ca_alho,"Output/chr/DP7000/chr16/CA_samples_with_inversions_chr16.csv")
ca_alho$Sample #13 individuals
