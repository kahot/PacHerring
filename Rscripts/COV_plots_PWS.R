...
for (w in 1: length(winsize)){
    ...
    for (p in 1: length(pops)){
        ...
        cov<-read.csv(paste0("~/Projects/Pacherring_Vincent/MD7000/PWSonly_maf05_temp_cov_matrix_",pops[p],"_",winsize[w],".csv"))
        cov<-cov[,-1]
        
        ci<-read.csv(paste0("~/Projects/Pacherring_Vincent/MD7000/PWSonly_maf05_",pops[p],"_Cov_CIs_bootstrap5000_",winsize[w],"window.csv"))
        ci<-ci[,-1]
    ...
    ggplot(data=covs, aes(x=year, y=value, color=location, shape=series))+
     ...
     ggsave(paste0("Output/COV/PWSonly_Cov_overtime_CI_",winsize[w],".window.pdf"),width = 4.7, height = 3)
}    
...
for (w in 1: length(winsize)){
    ...
    for (p in 1: length(pops)){
        ...
        #covariance output file
        cov<-read.csv(paste0("~/Projects/Pacherring_Vincent/MD7000/PWSonly_maf01_temp_cov_matrix_",pops[p],"_",winsize[w],".csv"))
        cov<-cov[,-1]
        
        ci<-read.csv(paste0("~/Projects/Pacherring_Vincent/MD7000/PWSonly_maf01_",pops[p],"_Cov_CIs_bootstrap5000_",winsize[w],"window.csv"))
        ci<-ci[,-1]
    ...
    ggsave(paste0("Output/COV/PWSonly_maf01_Cov_overtime_CI_",winsize[w],".window_new.pdf"),width = 4.7, height = 3)
}    
...
for (i in 1:length(pops)){
    vars1000<-read.csv(paste0("~/Projects/Pacherring_Vincent/MD7000/",pops[i],"only_vars_bootstrap_250k.csv"))
    #colnames(vars1000)<-c("1991-1996","1996-2006","2006-2017")
    varlabels<-read.csv(paste0("~/Projects/Pacherring_Vincent/MD7000/",pops[i],"only_varlabels.csv"))
    varlabels$nyears=varlabels$X1-varlabels$X2 
    
    #calculate Ne with bootstrapped variables (1000)
    ...
    # bootstrapped Ne values
    vardfm<-melt(var_df)
    vardfm$pop<-pops[i]
    
    var_dfm<-rbind(var_dfm,vardfm)
    
    #Estimated Ne from the method of moment approach  
    #Ne = -t/(2log(1-Var))
    
    df<-read.csv(paste0("~/Projects/Pacherring_Vincent/MD7000/PWSonly_maf05_PWS_Cov_CIs_bootstrap5000_100kwindow.csv"))
    df<-df[1:3,-1]
...
ave_est<-data.frame(aggregate(var_dfm["var"], by=list(var_dfm$year, var_dfm$pop), mean,na.rm=T))
colnames(ave_est)<-c("year","pop","mean")

ggplot(var_dfm, aes(x=var, color=year, fill=year))+
    geom_histogram( binwidth =5, alpha=0.6)+
    #facet_wrap(~pop, ncol=1)+xlim(0,2000)+
    xlab("Estimated Ne")+
    ylab("")+
...
ggsave("Output/COV/PWSonly_Ne_estimates_distribution.pdf", width = 7, height = 4)
