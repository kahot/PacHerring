...
pcacols<-c('#ffffb3','#8dd3c7','#bebada')
...
sfiles<-list.files("../Data/chr12/", pattern="^PH_chr12.+_stats")
Het<-data.frame()
Het_sum<-data.frame()
for (i in 1:length(sfiles)){
    ...
    df1$Group<-"Group1"
    df1$Group[df1$Sample %in% pca12$Sample[pca12$Group=="Group2"]]<-"Group2"
    df1$Group[df1$Sample %in% pca12$Sample[pca12$Group=="Group3"]]<-"Group3"
    #write.csv(df,paste0("../Output/chr12/chr12_Hetero_nonTB_",pname,"_maf05.csv"))
    df1$region<-gsub("PH_chr12_",'',pname)
    df1$pop<-"nonTB"
    Het<-rbind(Het, df1)
    
    #TB only
    df2<-df[grepl("TB",df$Sample),]
    df2$Group<-"Group1"
    df2$Group[df2$Sample %in% pca12$Sample[pca12$Group=="TB_Group2"]]<-"Group2"
    df2$Group[df2$Sample %in% pca12$Sample[pca12$Group=="TB_Group3"]]<-"Group3"
    #write.csv(df,paste0("../Output/chr12/chr7_Hetero_",pname,"_maf05.csv"))
    df2$region<-gsub("PH_chr12_",'',pname)
    df2$pop<-"TB"
    Het<-rbind(Het, df2)
    
    
    
    Ho<-aggregate(df1[,"Ho"], by=list(df1$Group), mean )
    He<-aggregate(df1[,"He"], by=list(df1$Group), mean )
    het<-cbind(Ho, He$x)
    colnames(het)<-c("Group","Ho","He")
    het$region<-gsub("PH_chr12_",'',pname)
    het$pop<-"nonTB"
    
    Ho2<-aggregate(df2[,"Ho"], by=list(df2$Group), mean )
    He2<-aggregate(df2[,"He"], by=list(df2$Group), mean )
    het2<-cbind(Ho2, He2$x)
    colnames(het2)<-c("Group","Ho","He")
    het2$region<-gsub("PH_chr12_",'',pname)
    het2$pop<-"TB"
    Het_sum<-rbind(Het_sum,het)
    Het_sum<-rbind(Het_sum,het2)
}
...
ggplot()+
    ...
    scale_color_manual(values=c("orange",gcols[2:3]))+
    scale_fill_manual(values=paste0(gcols, "66"))+xlab('')+
    scale_x_discrete(labels=c('Inversion',"Non-Inversion"))
ggsave("../Output/PCA/Chr12_Ho.in.twoRegions.png", width =6, height=3 , dpi=300)

```
![](../Output/PCA/Chr12_Ho.in.twoRegions.png)  

### Inversion heterozygotes: More common or less common?  
...
ggplot(dfm,aes(x=pop.yr, y=value, fill=variable))+
    ...
    theme_classic()+ylab("Heterozygosity")+xlab('')+
    theme(legend.title = element_blank(), axis.text.x = element_text(angle=45, hjust=1))+
    scale_fill_manual(values=c("#7570b3CC","#e7298aCC"))
ggsave("../Output/chr12/Expected_ovserved_hetero_inversion.png", height = 3.5, width = 6, dpi=300)   

# HWE (X2) test
summary$N<-rowSums(summary[,2:4])
summary$Exp.1<-summary$p^2*summary$N
summary$Exp.2<-summary$p*summary$q*2*summary$N
summary$Exp.3<-summary$q^2*summary$N

chi<-data.frame(pop=pops)
for (i in 1: length(pops)){
    ex<-summary[summary$pop.yr==pops[i],2:4]
    ex[2,]<-summary[summary$pop.yr==pops[i],10:12]
    re<-chisq.test(ex)
    chi$p.value[i]<-re$p.value
}

#chisqure test results - None are significant
#     pop   p.value
#1   TB91 0.8018526
#2   TB96 0.9566413
#3   TB06 0.8813503
...
#Aggregated test for all pops
colSums(summary[,2:4]) 
#Group1 Group2 Group3 
#   735    111     46 
colMeans(summary[,c("p","q")])
#       p        q 
#0.891861 0.108139 
0.891861^2*892 #709.5111
0.108139^2*892 #10.43109
2*0.891861*0.108139*892 #172.0578
chisq.test(c(735,111,46), c(710,172,10))
#X-squared = 6, df = 4, p-value = 0.1991

```
![](../Output/chr12/Expected_ovserved_hetero_inversion.png)  

#### -> Chiq-square test results were not significant for any population or all combined.


```{r message=FALSE, warning=FALSE,eval=FALSE}

### Heterozygosity in the rest of the chr12
# Calculate Ho and He from bcftools stats output files (stats from PWSonly files)
...
for (i in 1:length(sfiles)){
    ...
    df$He<-2*df$p*df$q
    df$Ho<-df$nHets/rowSums(df[,c("nRefHom","nNonRefHom","nHets")])
    df$pop.yr<-gsub("\\d*\\_", '',df$Sample)
    df$region<-gsub("PH_chr12_",'',pname)
    Het<-rbind(Het, df)
    
    Ho<-aggregate(df[,"Ho"], by=list(df$pop.yr), mean )
    He<-aggregate(df[,"He"], by=list(df$pop.yr), mean )
    het<-cbind(Ho, He$x)
    colnames(het)<-c("Pop","Ho","He")
    het$region<-gsub("PH_chr12_",'',pname)
    Het_sum<-rbind(Het_sum,het)
}

# Non-inversion Chr12: observed heterozygosity
ggplot(Het[Het$region=="noInversion",], aes(x=p, y=Ho, color=region))+
    geom_point()+xlab("Major allele freq. (p)")

ggplot(Het, aes(x=q, y=Ho, color=region))+
    geom_point()
...
colnames(tb)[4:5]<-colnames(pc7)[4:5]
pc7<-rbind(pc7,tb)
...
pca7$pop<-factor(pca7$pop, levels=c("TB","PWS","SS","BC","WA","CA"))


ggplot()+
    geom_point(data = pca7, aes(x = PC1, y = PC2, fill = pop, color = pop, shape = factor(year)), size = 3)+
    scale_shape_manual(values=c(23,25,3,3,21), name="Year")+
    scale_fill_manual(values=paste0(cols,"99"), guide="none")+
    scale_color_manual(values=cols, name="Population")+
    xlab(paste("PC 1: ", prop_explained[1],"%\n",sep = ""))+
    ...
    stat_ellipse(data=pca7[pca7$Group!="TB_Group3"&pca7$Group!="Group1",], aes(x=PC1, y=PC2, group=Group), level=0.99,color="gray50", size=0.3)+
    geom_ellipse(aes(x0=0.075, y0=0.07,  angle=45, a=0.02, b=0.022), level=0.999,size=0.05, color="gray70")+
    geom_ellipse(aes(x0=-0.023, y0=-0.003,  angle=-35, a=0.02, b=0.008), level=0.999,size=0.05, color="gray70")+
    #geom_path()+
...
ggsave("../Output/PCA/Chr7_PCA_withGroups.png", width = 5.5, height = 4.5, dpi=300)
...
```{r eval=FALSE, message=FALSE, warning=FALSE}
#Plot proportion for each population
ch7.2<-pca7[pca7$pop=="TB",]
ch7.1<-pca7[pca7$pop!="TB",]
...
"#E16A86" "#C7821C" "#909800" "#00A846" "#00AD9A" "#00A2D3" "#9183E6" "#D766C9"
...
p3<-ggvenn(x, fill_color = colors[c(5,8)], stroke_size = 0.5, set_name_size = 4,text_size=3)+ggtitle("Group3")
...
p2<-ggvenn(x2, fill_color = colors[c(5,8)],stroke_size = 0.5, set_name_size = 4,text_size=3)+ggtitle("Group2")
...
p1<-ggvenn(x1, fill_color = colors[c(5,8)],stroke_size = 0.5, set_name_size = 4,text_size=3)+ggtitle("Group1")
...
### ch7: Compare heterozygosity per region per group per year using bcftools
The following code is referred as "Box1"
```{bash eval=FALSE, echo=TRUE}
#Calculate heterozygosity per window using bcftools (het_stats_PWS91.sh)
# 
#subset chr12 without inversion into one 
vcftools --gzvcf Data/new_vcf/PH_DP600_7000_minQ20_minMQ30_NS0.5_maf05.vcf.gz --bed Data/chr12/chr7_withoutInversion.bed --out Data/new_vcf/PH_chr7_noInversion --recode --keep-INFO-all 

vcftools --gzvcf Data/new_vcf/PH_DP600_7000_minQ20_minMQ30_NS0.5_maf05.vcf.gz --bed Data/chr12/chr7_Inversion.bed --out Data/new_vcf/PH_chr7_Inversion --recode --keep-INFO-all 

gzip -c Data/new_vcf/PH_chr7_Inversion.recode.vcf > Data/new_vcf/PH_chr7_Inversion.vcf.gz
gzip -c Data/new_vcf/PH_chr7_noInversion.recode.vcf > Data/new_vcf/PH_chr7_noInversion.vcf.gz

bcftools stats -s - Data/new_vcf/PH_chr7_Inversion.vcf.gz | grep '^PSC' > Data/new_vcf/PH_chr7_Inversion_stats

bcftools stats -s - Data/new_vcf/PH_chr7_noInversion.vcf.gz | grep '^PSC' > Data/new_vcf/PH_chr7_noInversion_stats

```


### ch7: Results of observed heterozygosity (Ho) per year for each group
* Summarize Ho based on two regions (region1=Inversion vs. region2=Non-Inversion)
...
## Read the stats files and create het summary
# Calculate Ho and He from bcftools stats output files (stats from PWSonly files)
sfiles<-list.files("../Data/new_vcf/", pattern="^PH_chr7.+_stats")
Het<-data.frame()
Het_sum<-data.frame()
for (i in 1:length(sfiles)){
    df<-read.table(paste0("../Data/new_vcf/", sfiles[i]), sep="\t", header=F)
    pname<-gsub("_stats",'',sfiles[i])
    df<-df[,c(3:6)]
    colnames(df)<-c("Sample","nRefHom","nNonRefHom","nHets")
    df$p<-(2*df$nRefHom+df$nHets)/(rowSums(df[,c("nRefHom","nNonRefHom","nHets")])*2)
    ...
    df1<-df[!grepl("TB",df$Sample),]
    df1$Group<-"Group1"
    df1$Group[df1$Sample %in% pca7$Sample[pca7$Group=="Group2"]]<-"Group2"
    df1$Group[df1$Sample %in% pca7$Sample[pca7$Group=="Group3"]]<-"Group3"
    write.csv(df,paste0("../Output/chr12/chr7_Hetero_",pname,"_maf05.csv"))
    df1$region<-gsub("PH_chr7_",'',pname)
    df1$pop<-"nonTB"
    Het<-rbind(Het, df1)
    ...
    df2<-df[grepl("TB",df$Sample),]
    df2$Group<-"Group1"
    df2$Group[df2$Sample %in% pca7$Sample[pca7$Group=="TB_Group2"]]<-"Group2"
    df2$Group[df2$Sample %in% pca7$Sample[pca7$Group=="TB_Group3"]]<-"Group3"
    #write.csv(df,paste0("../Output/chr12/chr7_Hetero_",pname,"_maf05.csv"))
    df2$region<-gsub("PH_chr7_",'',pname)
    df2$pop<-"TB"
    Het<-rbind(Het, df2)
    ...
    het<-cbind(Ho, He$x)
    colnames(het)<-c("Group","Ho","He")
    het$region<-gsub("PH_chr7_",'',pname)
    het$pop<-"nonTB"
    ...
    het2<-cbind(Ho2, He2$x)
    colnames(het2)<-c("Group","Ho","He")
    het2$region<-gsub("PH_chr7_",'',pname)
    het2$pop<-"TB"
    Het_sum<-rbind(Het_sum,het)
    Het_sum<-rbind(Het_sum,het2)
    
}
write.csv(Het_sum, "../Output/chr7/chr7_Heteroz_Inv_summary.csv")
...
ggplot()+
    ...
    scale_fill_manual(values=paste0(gcols, "66"))+xlab('')+
    scale_x_discrete(labels=c('Inversion',"Non-Inversion"))
ggsave("../Output/PCA/Chr7_Ho.in.twoRegions.png", width =6, height=3 , dpi=300)

```

![](../Output/PCA/Chr7_Ho.in.twoRegions.png)
