...
![](../Output/Ne/Gone/PWS_recent_ne.by.Gone.png)
...
ggplot(neT, aes(x=Generation, y=Geometric_mean))+
    geom_line()+ylab("Ne")+theme_bw()+xlim(0,400)+
    ggtitle("TB17")
ggsave("../Output/Ne/Gone/SS_recent_ne.by.Gone.png", width = 6, height = 4, dpi=300)
```
![](../Output/Ne/Gone/SS_recent_ne.by.Gone.png)
...
ggplot(neall, aes(x=Generation, y=Geometric_mean, color=pop))+
    geom_line()+ylab("Ne")+theme_bw()+xlim(0,400)+ylim(4000000,5000000)
    ggtitle("1996 Data")+theme(legend.title = element_blank())
ggsave("../Output/Ne/Gone/3pop1996_recent_ne.by.Gone_yscaled.png", width = 6.5, height = 4, dpi=300)
#mostly same
```
