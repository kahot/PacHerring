...
library(ggforce)
library(tidyverse)
#library(PopGenome) #not compatible anymore with arm64e architecture
library(dplyr)
library(cowplot)
library(ggvenn)
library(knitr)
...
#2007 vs. 2017
fisher.test(ch15_summary[c(3,4),2:4])
#p-value = 0.06718
# chi-square test: X-squared = 5.4156, df = 2, p-value = 0.06668

#91 vs 07
fisher.test(ch15_summary[c(1,3),2:4])
#p-value = 0.08019
#96 vs/ 97
fisher.test(ch15_summary[c(2,3),2:4])
#p-value = 0.7486

#In cases where the expected value is found to be small, the normal approximation of the multinomial distribution can fail, and in such cases it is found to be more appropriate to use the G-test, a likelihood ratio-based test statistic
...
g.test(ch15_summary[,2:4])
#X-squared = 11.231, p-value = 0.08151

#Fisher's exact test (Significance in G-test is approximate ) 
fisher.test(ch15_summary[,2:4])
#p-value = 0.09765
#alternative hypothesis: two.sided

# 2007 vs. 2017
g.test(ch15_summary[c(3,4),2:4])
#X-squared = 5.9906, p-value = 0.05002
...
# 1991 vs. 1996
g.test(ch15_summary[c(1,2),2:4])
#X-squared = 4.6045, p-value = 0.1
g.test(ch15_summary[c(2,3),2:4])
#X-squared = 0.73175, p-value = 0.6936
g.test(ch15_summary[c(1,4),2:4])
#X-squared = 0.17875, p-value = 0.9145
g.test(ch15_summary[c(1,3),2:4])
#X-squared = 5.5783, p-value = 0.06147
g.test(ch15_summary[c(2,4),2:4])
#X-squared = 5.5927, p-value = 0.06103
...
ggplot(c15m, aes(x=factor(year), y=value, fill=variable))+
    ...
    theme_linedraw()+theme(legend.title = element_blank(), panel.grid.minor=element_blank(), panel.grid.major.x=element_blank())+
    ylab("Proportion of individuals")+
    scale_fill_manual(values=paste0(gcols,"CC"))+xlab("Year")+ggtitle("PWS Chr15")
ggsave("../Output/PCA/PWS_ch15_prop_indivi_in3groups.png", width = 6, height = 4, dpi=150)


#Do logistic regression with group, year and interaction
df<-c15m
colnames(df)[2:3]<-c("group","count")
df$group<-as.integer(df$group)
df$year<-str_replace_all(df$year, c("1991"="1","1996"="2","2007"="3","2017"="4"))
df$year<-as.integer(df$year)
mod1<-glm(count~year+group+year*group, data=df)
summary(mod1)

#glm(formula = count ~ year + group + year * group, data = df)
#
#Deviance Residuals: 
#   Min      1Q  Median      3Q     Max  
#-7.583  -4.550   1.342   2.967   8.583  
#
#Coefficients:
#            Estimate Std. Error t value Pr(>|t|)   
#(Intercept)   47.500     12.436   3.820  0.00509 **
#year          -3.267      4.541  -0.719  0.49239   
#group        -12.750      5.757  -2.215  0.05765 . 
#year:group     1.100      2.102   0.523  0.61495   

mod2<-glm(count~year+group, data=df)
summary(mod2)

#            Estimate Std. Error t value Pr(>|t|)    
#(Intercept)   42.000      6.374   6.590  0.00010 ***
#year          -1.067      1.646  -0.648  0.53306    
#group        -10.000      2.253  -4.438  0.00163 **
...
* reference: https://evolutionarygenetics.github.io/Chapter8.html
...
ggplot()+
    ...
    scale_color_manual(values=c("orange",gcols[c(2,3)]))+
...
![](../Output/PCA/PCA_Chr15.png){width=40%}  ![no TB](../Output//PCA/PCA_noTB_Chr15.png){width=40%}  
...
ggplot(data=pop.sum, aes(x=yr.pop, y=n, fill=factor(Group)))+
    ...
    scale_fill_manual(values=gcols, labels=c("Group1","Group2","Group3"))+
    theme_light()+ggtitle("Chr15")+
    xlab("")+ylab("Proportion")+theme(legend.title = element_blank())
ggsave("../Output/PCA/Chr15_3groups_barplot_allPops.png", width = 7, height = 4.5, dpi=300)
```
...
* PWS populations are uniquely different from SS pops.
<br>
...
fisher.test(ch8_summary[,2:4])
#p-value = 0.1172

chisq.test(ch8_summary[c(2,4),2:4]) #PWS96 vs. PWS17
#X-squared = 8.9581, df = 2, p-value = 0.01134
fisher.test(ch8_summary[c(2,4),2:4])
#p-value = 0.007934


g.test(ch8_summary[,2:4])
#X-squared = 9.4156, p-value = 0.1515
g.test(ch8_summary[c(2,4),2:4])
#X-squared = 9.0071, p-value = 0.01107
...
ggplot(c8m, aes(x=factor(year), y=value, fill=variable))+
    ...
    scale_fill_manual(values=gcols)+xlab("Year")+ggtitle("PWS Chr8")
...
![](../Output/PCA/PCA_Chr8.png){width=40%} ![](../Output/PCA/PCA_noTB_Chr8.png){width=40%}
...
ggplot(data=pop.sum, aes(x=yr.pop, y=n, fill=factor(Group)))+
    ...
    scale_fill_manual(values=gcols, labels=c("Group1","Group2","Group3"))+
    theme_light()+ggtitle("Chr 8")+
    xlab("")+ylab("Proportion of individuals")+theme(legend.title = element_blank())
...
p3<-ggvenn(x, stroke_size = 0.5, set_name_size = 4,text_size=3)+ggtitle("Group3")

c15g2<-c15$Sample[c15$Group=="Group2"]
c8g2<-c8$Sample[c8$group=="Group2"]
x2<-list(chr8=c8g2,chr15=c15g2)
p2<-ggvenn(x2, stroke_size = 0.5, set_name_size = 4,text_size=3)+ggtitle("Group2")
...
![All populations](../Output/PCA/PCA_Chr20.png){width=40%} ![No TB ](../Output/PCA/PCA_noTB_Chr20.png){width=40%}    
...
fisher.test(ch20_summary[c(2,3),2:4]) 
#p-value = 0.04553

library(chisq.posthoc.test)
#Run post-hoc analysis
chisq.posthoc.test(t(X), method = "holm")
...
ggplot(c20m, aes(x=factor(year), y=value, fill=variable))+
    geom_bar(stat="identity", width=0.5, color="gray40",position = "fill")+
    theme_linedraw()+theme(legend.title = element_blank())+ylab("Proportion of individuals")+
    scale_fill_manual(values=gcols)+xlab("Year")+ggtitle("PWS Chr20")
...
![](../Output/PCA/PCA_Chr20.png){width=40%}![](../Output/PCA/PCA_noTB_Chr20.png){width=40%}

### Compare 3 groups separated by PC1 ('Inversion Grouping')
...
ggplot(chr20_sum, aes(x=yr.pop, y=Freq, fill=Group))+
    ...
    theme_linedraw()+theme(legend.title = element_blank())+ylab("Proportion of individuals")+
    scale_fill_manual(values=gcols)+xlab("")+ggtitle("Chr20")
...
### How the grouping overlap between 'all-pop (w/o TB)' vs. 'PWSonly'?   
* Groups separated by PC1 = Clusters by an inversion
* Groups separated by PC2 in PWS = Groups
...
#
g3_p[g3_p %in% g2_a] #only 3 samples from Group3_PWS belong to group2-All
...
ggplot(chr20_cl, aes(x=yr.pop, y=Freq, fill=Cluster))+
    geom_bar(stat="identity", width=0.8, color="gray40",position = "fill")+
    theme_linedraw()+theme(legend.title = element_blank())+ylab("Proportion of individuals")+
    scale_fill_manual(values=gcols)+xlab("")+ggtitle("Chr20 Groping (non-inversion)")
ggsave("../Output/PCA/Ch20_prop_indivi_in3groups.nonInversion.png", width = 7, height = 4, dpi=300)

"#ffffb3" "#8dd3c7" "#bebada"

#create colors based on cluster
ccols<-c("#ffffd4","#fed98e","#fe9929","#e5f5f9","#99d8c9","#2ca25f","#efedf5","#bcbddc","#756bb1")
...
![](../Output/PCA/Ch20_prop_indivi_in3groups.nonInversion.png)

![](../Output/PCA/Ch20_prop_indivi_inClusters.png)
...
pca<-read.csv("../Output/PCA/pca_chr20.csv")
pca$yr.pop<-paste0(pca$pop,substr(pca$year, 3,4))
#TB
#Assign the inversion group
pca1<-pca[pca$pop=="TB",]
tb_gp3<-pca1[pca1$PC2<=(-0.04), ] #26samples
tb_gp1<-pca1[pca1$PC2>=(0.0125), ] #133
sampl<-c(tb_gp1$Sample, tb_gp3$Sample)
tb_gp2<-pca1[!(pca1$Sample %in% sampl),  ] #122

tb_gp3$Group="Group3"
tb_gp2$Group="Group2"
tb_gp1$Group="Group1"
tb_20<-rbind(tb_gp1, tb_gp2,tb_gp3)
write.csv(tb_20, "../Output/PCA/chr20_group_TB.csv")

#non_TB
pca20<-read.csv("../Output/PCA/chr20_group_cluster.csv")
#Group is the inversion clustring

#Plot proportion for each population
pop.sum<-pca20 %>% count(Group, yr.pop)
pop.sum$yr.pop<-factor(pop.sum$yr.pop, levels=c("PWS91","PWS96","PWS07","PWS17","SS96","SS06","SS17","BC17","WA17","CA17"))

pop.sum2<-tb_20 %>% count(Group, yr.pop)
...
full1<-ggdraw()+
    ...
    draw_plot_label(c("no TB", "TB"), x=c(0.275, 0.72), y=c(1, 1), size = 10)
save_plot(filename ="../Output/PCA/ch20_prop_in3groups.png", plot = full1,base_width = 10, base_height = 5, dpi=300)   
```

![](../Output/PCA/ch20_prop_in3groups.png)
...
![](../Output/PCA/PCA_Chr4.png){width=30%} ![](../Output/PCA/PCA_noTB_Chr4.png){width=30%}
...
ggplot(c4m, aes(x=factor(year), y=value, fill=variable))+
    geom_bar(stat="identity", width=0.5, color="gray40",position = "fill")+
    theme_linedraw()+theme(legend.title = element_blank())+ylab("Proportion of individuals")+
    scale_fill_manual(values=gcols)+xlab("Year")+ggtitle("PWS Chr4")
...
![](../Output/PCA/PWS_Chr15_Ho.in.tworegions_byYears.png)

![](../Output/PCA/PWS_Chr20_Ho.in.tworegions.png)
...
#colors
colors8<-qualitative_hcl(n=8, palette="Set3")
x<-list(chr4=c4g3, chr8=c8g3,chr15=c15g3, chr20=c20g3)
p3<-ggvenn(x, stroke_size = 0.5,fill_color = colors8[c(2,4,6,8)], set_name_size = 4,text_size=2.5)+ggtitle("Group3")
...
p2<-ggvenn(x2, stroke_size = 0.5, fill_color = colors8[c(2,4,6,8)], set_name_size = 4,text_size=2.5)+ggtitle("Group2")
...
p1<-ggvenn(x1, stroke_size = 0.5, fill_color = colors8[c(2,4,6,8)],set_name_size = 4,text_size=2.5)+ggtitle("Group1")
...
p3<-ggvenn(x.1, stroke_size = 0.5, set_name_size = 4,text_size=2.5)+ggtitle("Group3")

x2.2<-list(chr4=c4g2, chr8=c8g2)
p2<-ggvenn(x2.2,  stroke_size = 0.5, set_name_size = 4,text_size=2.5)+ggtitle("Group2")
x1.1<-list(chr4=c4g1, chr8=c8g1)
p1<-ggvenn(x1.1, stroke_size = 0.5, set_name_size = 4,text_size=2.5)+ggtitle("Group1")
...
p3<-ggvenn(x.1, fill_color = cols, stroke_size = 0.5, set_name_size = 4,text_size=2.5)+ggtitle("Group3")
x2.2<-list(chr4=c4g2, chr15=c15g2)
p2<-ggvenn(x2.2, fill_color = cols, stroke_size = 0.5, set_name_size = 4,text_size=2.5)+ggtitle("Group2")
x1.1<-list(chr4=c4g1, chr15=c15g1)
p1<-ggvenn(x1.1, fill_color = cols, stroke_size = 0.5, set_name_size = 4,text_size=2.5)+ggtitle("Group1")
...
p3<-ggvenn(x, fill_color = colors8[c(2,4,6,8)], stroke_size = 0.5, set_name_size = 4,text_size=3)+ggtitle("Group3")

c20c2<-pca20$Sample[pca20$Group=="Group2"]
x2<-list(chr4=c4g2, chr8=c8g2,chr15=c15g2, chr20=c20c2)
p2<-ggvenn(x2, fill_color =  colors8[c(2,4,6,8)], stroke_size = 0.5, set_name_size = 4,text_size=3)+ggtitle("Group2")

c20c1<-pca20$Sample[pca20$Group=="Group1"]
x1<-list(chr4=c4g1, chr8=c8g1,chr15=c15g1, chr20=c20c1)
p1<-ggvenn(x1, fill_color =  colors8[c(2,4,6,8)], stroke_size = 0.5, set_name_size = 4,text_size=3)+ggtitle("Group1")
...
# Chr16 Analysis 
![](../Output/PCA/PCA_Chr16.png) {width=40%}
...
#chr16 PCA info
pca<-read.csv("../Output/PCA/pca_chr16.csv")
pca$yr.pop<-paste0(pca$pop,substr(pca$year, 3,4))

#TB
#Assign the inversion group
pca1<-pca[pca$pop=="TB",]
tb_gp3<-pca1[pca1$PC2>(0.1), ] #3samples
tb_gp1<-pca1[pca1$PC2<=(0.025), ] #205
sampl<-c(tb_gp1$Sample, tb_gp3$Sample)
tb_gp2<-pca1[!(pca1$Sample %in% sampl),  ] #63

tb_gp3$Group="Group3"
tb_gp2$Group="Group2"
tb_gp1$Group="Group1"
tb_16<-rbind(tb_gp1, tb_gp2,tb_gp3)
write.csv(tb_16, "../Output/PCA/chr16_group_TB.csv")

#non_TB
pca16<-read.csv("../Output/chr/DP7000/chr16_PCAgroups.csv", row.names = 1)
#Group number is flipped.
pca16$Group2<-pca16$Group
pca16$Group[pca16$Group2==1]<-3
pca16$Group[pca16$Group2==3]<-1

#Plot proportion for each population
pop.sum<-pca16 %>% count(Group, yr.pop)
pop.sum$yr.pop<-factor(pop.sum$yr.pop, levels=c("PWS91","PWS96","PWS07","PWS17","SS96","SS06","SS17","BC17","WA17","CA17"))

pop.sum2<-tb_16 %>% count(Group, yr.pop)
pop.sum2$yr.pop<-factor(pop.sum2$yr.pop, levels=c("TB91","TB96","TB06","TB17"))
...
save_plot(filename ="../Output/PCA/ch16_prop_in3groups.png", plot = full1,base_width = 10, base_height = 5, dpi=300)   
```

![](../Output/PCA/ch16_prop_in3groups.png)
