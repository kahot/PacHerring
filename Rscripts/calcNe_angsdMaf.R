...
# trim to loci that meet unlinked filter
...
#dat07 <- dat07[unlnk, nomatch=0] # nomatch=0 so that non-matching rows are dropped
...
################################
setnames(dat96, c("knownEM", 'nInd'), c("freq1", 'nInd1'))
setnames(dat07, c("knownEM", 'nInd'), c("freq2", 'nInd2'))
...
dat9607 <- dat07[dat96, .(chromo, position, freq1, freq2, nInd1, nInd2)][!is.na(freq1) & !is.na(freq2) & freq1 > 0.1 & freq2 > 0.1, ]
...
nrow(dat9607) # 94976 (w/out unplaced), 95426 (w/ unplaced)
...
dat9607[, jrNe2(freq1, freq2, nInd1, nInd2, 11)] 
#596.1891
...
lgs <- dat9607[, sort(unique(chromo))]

boot9607lg <- boot(lgs, jrNe2block, 4000, gen = 11, alldata = dat9607)
print(boot9607lg)
median(boot9607lg$t[is.finite(boot9607lg$t)]) # median bootstrap
boot.ci(boot9607lg, type = c('perc'))
